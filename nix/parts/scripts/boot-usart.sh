...
# Read from local out directory
BUNDLE="${BUNDLE:-${PWD}/out}"
SPL_NAME="phase1-spl.bin"
PAYLOAD_NAME="phase2-payload.bin"
...
sx -vv --xmodem "${BUNDLE}/flash-targets/${SPL_NAME}" <>"${TTY}" 1>&0

echo "Phase 1 complete. Wait ${SLEEP_TIME} seconds for DDR..."
sleep "${SLEEP_TIME}"

echo "Starting Phase 2: Uploading ${PAYLOAD_NAME}..."
sb -vv --ymodem "${BUNDLE}/flash-targets/${PAYLOAD_NAME}" <>"${TTY}" 1>&0
